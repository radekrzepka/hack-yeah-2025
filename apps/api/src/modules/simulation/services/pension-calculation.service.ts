...
export interface SimulationInput {
  ...
  postalCode?: string;
  customExperience?: Array<CustomExperiencePeriod>;
}
...
export class PensionCalculationService {
  ...
  async calculatePension(
    ...
    for (let year = workStartYear; year < retirementYear; year++) {
      ...
      // Check if this year falls within a custom experience period
      const customPeriod = this.findCustomExperienceForYear({
        year,
        customExperience: input.customExperience,
      });

      let estimatedYearlySalary: number;
      let currentContributionRates: {
        total: number;
        mainAccount: number;
        subAccount: number;
      };
      ...
      if (customPeriod) {
        // Use custom salary and contract type
        estimatedYearlySalary = customPeriod.monthlySalary * 12;
        currentContributionRates = this.getContributionRates(
          customPeriod.contractType,
        );
      } else {
        // Apply wage growth projection if enabled
        estimatedYearlySalary = this.dataProvider.estimateSalaryForYear({
        ...
        // Use default contract-specific contribution rates
        currentContributionRates = contributionRates;
      }

      lastYearlySalary = estimatedYearlySalary;

      // Use contract-specific contribution rates (either custom or default)
      const subAccountRate = currentContributionRates.subAccount;
      const mainAccountRate = currentContributionRates.mainAccount;
    ...
    if (input.includeSickLeave) {
      ...
      const absencePercentage =
        await this.dataProvider.getAbsenceDataForPostalCode(
          input.postalCode || "",
          input.sex,
        );
  ...
  private findCustomExperienceForYear(params: {
    year: number;
    customExperience?: Array<CustomExperiencePeriod>;
  }): CustomExperiencePeriod | undefined {
    const { year, customExperience } = params;
    if (!customExperience || customExperience.length === 0) {
      return undefined;
    }
    return customExperience.find(
      (period) => year >= period.yearStart && year <= period.yearEnd,
    );
  }

  private getContributionRates(
    contractType: "uop" | "b2b" | "zlecenie" | "dzielo",
  ): {
    total: number;
    mainAccount: number;
    ...
    switch (contractType) {
      case "uop":
        return {
          ...
          subAccount: 0.073, // 7.3% (lub 0.0438 ZUS + 0.0292 OFE, jeśli wybrano)
        };
      case "b2b":
        return {
